function [bestSolution, bestFitness, iteration] = dfdb_lshade_case_4(p, dimension, maxIteration)
...
%%      if nfes > max_nfes; exit(1); end
	...
	pos = find(sf <= 0);
      ...
      fdb_index = dFDB(pop, fitness,10,nfes,max_nfes);
      fdb_percent = rand();
      pNP = max(round(p_best_rate * pop_size), 2); %% choose at least two best solutions
      if fdb_percent > 0.15 &&  nfes < 0.25*max_nfes
        randindex = ceil(rand(1, pop_size) .* fdb_index); %% select from [1, 2, 3, ..., pNP]
      else
      ...
      if nfes < 0.25*max_nfes
         vi = pop + sf(:, ones(1, problem_size)) .* (pbest - pop + pop(fdb_index, :) - popAll(r2, :));  
      else
      ...
      children_fitness = testFunction(ui', p);
      [mS, ~] = size(children_fitness);
        if mS == 1
